...
export default function CryptoPriceTable() {
  ...
  } = useSWR<CoinPricesType[]>('/api/coin-prices', GenericFetcher, {
    refreshInterval: 100000,
  });

  // Conditionally render this component
  if (coinsError) {
    return <div className="p-4 text-red-500">Error Loading Coins Data...</div>;
  } else if (loadingCoins) {
    return <div className="p-4 text-gray-300">Loading Coins Data Table...</div>;
  } else {
    ...
    return (
      <div className="mt-10 bg-gray-900 p-4 shadow-lg">
        ...
        <Input
          ...
          className="mb-4 border-gray-700 bg-gray-800 text-gray-100 focus:border-blue-500 focus:ring-blue-500"
        />
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {filteredCoinsData?.map((coin) => (
              <TableRow key={coin.name} className="border-b border-gray-800">
                ...
                <TableCell className="text-gray-300">
                  {Number(coin.current_price) < 0.01
                    ? '<$0.01'
                ...
                <TableCell className="text-gray-300">
                  {Number(coin.high_24h) < 0.01
                    ? '<$0.01'
                    : '$' + Number(coin.high_24h).toFixed(2)}
                </TableCell>
                <TableCell className="text-gray-300">
                  {Number(coin.low_24h) < 0.01
                    ? '<$0.01'
                ...
                <TableCell className="text-gray-300">
                  {'$' + Number(coin.total_volume).toFixed(2)}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
