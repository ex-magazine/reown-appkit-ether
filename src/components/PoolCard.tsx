...
const PoolCard = ({ data, mode, address }) => {
  ...
  const detectWinner = async () => {
    ...
    if (req) {
      setLoadingBtn(true);
      enqueueSnackbar(`Called smartcontract, detecting winner...`, {
        variant: 'success',
      });
  ...
  return (
    <Box className="pool-card">
      ...
      <Typography variant="h6" sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        {price > 0 && (
          <em style={{ color: '#bbb', fontSize: '14px' }}>
            {' '}
            (${p.toFixed(1)})
          </em>
        )}
      ...
      <Stack direction="row" spacing={2}>
        ...
        {pool.length < 10 && (
          <Button variant="contained" onClick={addStake} loading={loadingBtn}>
        ...
        <Button
          ...
          onClick={() => {
            ...
            enqueueSnackbar('Copied address of contract', {
              variant: 'success',
            });
          }}
        >
        ...
        <Button variant="contained" href={getLink()} target="_blank">
          <img
            src="/icons/block-explorer.png"
            style={{ width: '36px', height: '36px' }}
          />
        </Button>

        {isCreator && (
          <Button variant="contained" color="secondary" onClick={claimRewards}>
            Rewards
          </Button>
        )}
      </Stack>
