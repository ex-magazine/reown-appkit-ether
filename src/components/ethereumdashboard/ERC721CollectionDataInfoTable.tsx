...
export default function ERC721CollectionDataInfoTable(props: { address: string }) {
  ...
  const { data, error, isLoading } =
    useSWR(['/api/erc721-collection-data', { address }], ([url, body]: [string, PostFetcherArgumentsType]) => PostFetcher(url, { arg: body }), { refreshInterval: 100000 });
  ...
  if (isLoading) {
    ...
    return (
      <div className="p-4 bg-gray-900 mt-10 shadow-lg">
        <h2 className="text-2xl font-bold mb-4 text-gray-100">Collection Information</h2>
        <Table>
          <TableHeader>
            <TableRow>
