...
import PostFetcher from '@/app/ethereumdashboard/utils/functions/PostFetcher';
import {
  Table,
  TableBody,
  TableCell,
...
export default function ENSResolverInfoTable(props: { data: string }) {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<{ results: ENSResolverType[] }>(
  ...
  if (isLoading) {
    ...
    return (
      <div className="mt-10 bg-gray-900 p-4 shadow-lg">
        <h2 className="mb-4 text-2xl font-bold text-gray-100">ENS Resolver</h2>
        <Table>
          <TableHeader>
            <TableRow>
