    ...
    const response = await tmdbClient.get<IApiResponse<IMovie[]>>(
      '/discover/movie',
      {
        params: {
          ...
          ...(props.with_original_language && {
            with_original_language: props.with_original_language,
          }),
          ...(props.primary_release_year && {
            primary_release_year: props.primary_release_year,
          }),
...
export const getMovieInfo = async (id: string) => {
  try {
    ...
    const similarMovies = await tmdbClient.get<IApiResponse<IMovie[]>>(
      `/movie/${id}/similar`,
      {
        params: { language: 'en-US' },
      },
