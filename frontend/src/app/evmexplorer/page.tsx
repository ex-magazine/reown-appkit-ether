...
const Home: NextPage = () => {
  ...
  async function validatesSearch(input: string) {
    if (input.endsWith('.ens')) {
      ...
      if (address) {
        router.push(`/evmexplorer/contracts/${chain.value}/${address}`);
        return;
      }
    }
    if (input.length === 42) {
      router.push(`/evmexplorer/contracts/${chain.value}/${search}`);
      return;
    }

    if (input.length === 66) {
      router.push(`/evmexplorer/transactions/${chain.value}/${search}`);
      return;
    }

    if (input) {
      if (isFinite(Number(input))) {
        router.push(`/evmexplorer/blocks/${chain.value}/${search}`);
      } else {
