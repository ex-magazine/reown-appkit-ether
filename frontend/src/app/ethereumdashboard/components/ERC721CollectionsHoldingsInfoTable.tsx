...
export default function ERC721CollectionsHoldingsInfoTable(props: {
  data: ERC721CollectionsHoldingsType[];
}) {
  const { data } = props;
          ...
          {data?.map((collectionHolding, index: number) => (
            <TableRow key={index} className="border-b border-gray-800">
              <TableCell className="text-gray-300">
                {collectionHolding.name}
              </TableCell>
              <TableCell className="text-gray-300">
                {collectionHolding.symbol}
              </TableCell>
              <TableCell className="text-gray-300">
                {collectionHolding.token_address}
              </TableCell>
              <TableCell className="text-gray-300">
                {collectionHolding.contract_type}
              </TableCell>
              <TableCell className="text-gray-300">
                {collectionHolding.verified_collection ? 'YES' : 'NO'}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
