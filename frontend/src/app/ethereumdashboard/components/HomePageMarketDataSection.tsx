...
export default function HomePageMarketDataSection() {
  ...
  } = useSWR('/api/ethereumdashboard/global-market-data', GenericFetcher, {
  ...
  } = useSWR('/api/ethereumdashboard/global-defi-data', GenericFetcher, {
    refreshInterval: 50000,
  });
  ...
  if (marketError || defiError) {
    ...
    return (
      <div className="bg-gray-900 p-6 shadow-lg">
        <h2 className="mx-auto mb-6 text-2xl text-gray-100">Market Overview</h2>
              ...
              <div className="space-y-2">
                <p>
                  ...
                  <span className="text-gray-100">
                    {marketTableData?.active_cryptocurrencies}
                  </span>
                </p>
                <p>
                  Number of Exchanges:{' '}
                  <span className="text-gray-100">
                    {marketTableData?.markets}
                  </span>
                </p>
                <p>
                  Total Market Cap:{' '}
                  <span className="text-gray-100">
                    {'$' +
                      Number(marketTableData?.total_market_cap?.usd).toFixed(2)}
                  </span>
                </p>
                <p>
                  Market Dominance:
            ...
            <CardContent className="text-gray-300">
              <div className="space-y-2">
                <p>
                  DeFi Market Cap:{' '}
                  <span className="text-gray-100">
                    {'$' + Number(data?.defi_market_cap).toFixed(2)}
                  </span>
                </p>
                <p>
                  ETH Market Cap:{' '}
                  <span className="text-gray-100">
                    {'$' + Number(data?.eth_market_cap).toFixed(2)}
                  </span>
                </p>
                <p>
                  Trading Volume:{' '}
                  <span className="text-gray-100">
                    {'$' + Number(data?.trading_volume_24h).toFixed(2)}
                  </span>
                </p>
                <p>
                  Top Coin Name:{' '}
                  <span className="text-gray-100">{data?.top_coin_name}</span>
                </p>
                <p>
                  Top Coin DeFi Dominance:{' '}
                  <span className="text-gray-100">
                    {Number(data?.top_coin_defi_dominance).toFixed(2) + '%'}
                  </span>
                </p>
              </div>
            </CardContent>
