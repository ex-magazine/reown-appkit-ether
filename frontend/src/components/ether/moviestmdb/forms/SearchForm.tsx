...
import { debounce } from '@/lib/ether/moviestmdb/debounce';
...
const SearchForm = () => {
  ...
  const handleSearch = (values: typeof defaultValues) => {
    ...
    router.push(`/moviestmdb/search?query=${encodeURIComponent(trimSearch)}`);
  };

  const debouncedSearch = useCallback(debounce(handleSearch, 500), []);

  return (
    <form onSubmit={handleSubmit(debouncedSearch)} className="mx-4 flex-1">
      <Input
        type="text"
